...
public class PostGenerico extends AbstractHTTP{
	...
	protected Object procesarPeticionOK(BufferedReader in, AbstractJsonRestEstructura json, int responseCode) throws Exception {
        ...
        JSONObject jsonRespuesta= JsonUtils.convertir(in);
        String id=jsonRespuesta.get(r.getIdIteracion()).toString();
        String cabecera;
        ...
        CSVHandler.getInstance().escribirCSV(fichero, id+r.getSeparadorCSV()+json.getLine(), r.getIdIteracion()+r.getSeparadorCSV()+cabecera,true);        
